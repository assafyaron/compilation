...
public class AST_VAR_DEC_NEW_EXP_ARGS extends AST_VAR_DEC
{
    ...
    public TEMP IRme()
    {
        IR.getInstance().Add_IRcommand(new IRcommand_Allocate(variable,IR.getInstance().currLine));

		if (new_exp != null){
            
            int offset = IR.getInstance().offsetTable.getTopFrame().pushEntry(variable,false);
            if (new_exp instanceof AST_NEW_TYPE) {
                AST_NEW_TYPE newType = (AST_NEW_TYPE) new_exp;
                
                if (IR.getInstance().currentClassName != null && IR.getInstance().currentFuncName != null) {
				    IR.getInstance().Add_IRcommand(new IRcommand_Store(variable,newType.IRme(variable),IR.getInstance().currLine,offset,false,false,true));
                } else {
                    IR.getInstance().Add_IRcommand(new IRcommand_Store(variable,newType.IRme(variable),IR.getInstance().currLine,offset,false,false,false));
                }

            } else {

                if (IR.getInstance().currentClassName != null && IR.getInstance().currentFuncName != null) {
				    IR.getInstance().Add_IRcommand(new IRcommand_Store(variable,new_exp.IRme(),IR.getInstance().currLine,offset,false,false,true));
                } else {
                    IR.getInstance().Add_IRcommand(new IRcommand_Store(variable,new_exp.IRme(),IR.getInstance().currLine,offset,false,false,false));
                }
