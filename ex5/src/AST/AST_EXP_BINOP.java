...
public class AST_EXP_BINOP extends AST_EXP
{
	...
	public TEMP IRme()
	{
		...
		if (OP == 1) {
			IR.
			getInstance().
			Add_IRcommand(new IRcommand_Binop_Sub_Integers(dst,t1,t2,IR.getInstance().currLine));
		}
		if (OP == 2) {
			IR.
			getInstance().
		...
		if (OP == 6) {
			if (this.isString) {
				IR.
				getInstance().
				Add_IRcommand(new IRcommand_Binop_EQ_Strings(dst,t1,t2,IR.getInstance().currLine));
			} else {
			IR.
			getInstance().
