...
public class AST_CLASS_DEC_EXTENDS extends AST_CLASS_DEC
{
    ...
    public TEMP IRme() 
    {
        IR.getInstance().currentClassName = this.className;
        IR.getInstance().extendMap.put(this.className, this.superClassName);


        List<Field> arr = new ArrayList<>();
        IR.getInstance().classFieldsMap.put(this.className, arr);
        IR.getInstance().offsetTable.addScope();

        TEMP dst = TEMP_FACTORY.getInstance().getFreshTEMP();
        String label_start = IRcommand.getFreshLabel("class"+this.className+"_start");
        String label_end = IRcommand.getFreshLabel("class"+this.className+"_end");

        IRcommand lbl_strt_cmd = new IRcommand_Label(label_start,IR.getInstance().currLine,true,false,true,null,false);
		IR.
		getInstance().
		Add_IRcommand(lbl_strt_cmd);

        if (classBody == null) {
            IR.getInstance().Add_IRcommand(new IRcommand_New_Class(dst,className,0,IR.getInstance().currLine,superClassName));
        } else {
            classBody.IRme();
            IR.getInstance().Add_IRcommand(new IRcommand_New_Class(dst,className,0,IR.getInstance().currLine,superClassName));
        }
        IRcommand lbl_end_cmd = new IRcommand_Label(label_end,IR.getInstance().currLine,false,false,true,null,false);
		IR.
		getInstance().
		Add_IRcommand(lbl_end_cmd);

        IR.getInstance().offsetTable.popScope();
        IR.getInstance().currentClassName = null;
