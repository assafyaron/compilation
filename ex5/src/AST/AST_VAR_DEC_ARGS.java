	...
	public TEMP IRme()
	{
		...
		if (exp != null){
			...
			if (exp instanceof AST_EXP_INT) {
				AST_EXP_INT expInt = (AST_EXP_INT)exp;
				offset = IR.getInstance().offsetTable.getTopFrame().pushEntry(varName,expInt.i,false);
			} else if (exp instanceof AST_EXP_MINUS_INT) {
				AST_EXP_MINUS_INT expMinusInt = (AST_EXP_MINUS_INT)exp;
				offset = IR.getInstance().offsetTable.getTopFrame().pushEntry(varName,-expMinusInt.i,false);
			} else if (exp instanceof AST_EXP_STRING) {
				AST_EXP_STRING expString = (AST_EXP_STRING)exp;
				offset = IR.getInstance().offsetTable.getTopFrame().pushEntry(varName,expString.value,false);
			} else {
				offset = IR.getInstance().offsetTable.getTopFrame().pushEntry(varName,false);
			}

			String className = null;
			if (exp instanceof AST_EXP_VAR) {
				AST_EXP_VAR expVar = (AST_EXP_VAR) exp;
				System.out.println(expVar.var.getVarName());
				className = IR.getInstance().offsetTable.getClassName(expVar.var.getVarName());
				if (className != null) {
					IR.getInstance().offsetTable.getTopFrame().varToClassMap.put(varName,className);
					System.out.println("Adding to varToClassMap: " + varName + " -> " + className);
				}
			}

			if (t instanceof AST_TYPE_ID) {
				AST_TYPE_ID typeId = (AST_TYPE_ID)t;
				typeId.IRme(varName,false, className);
			}

			if (IR.getInstance().currentClassName != null && IR.getInstance().currentFuncName != null) {
				IR.getInstance().Add_IRcommand(new IRcommand_Store(varName,exp.IRme(),IR.getInstance().currLine, offset, false, false, true));
			} else {
				IR.getInstance().Add_IRcommand(new IRcommand_Store(varName,exp.IRme(),IR.getInstance().currLine, offset, false, false, false));
			}
