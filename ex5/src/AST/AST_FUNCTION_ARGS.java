...
public class AST_FUNCTION_ARGS extends AST_FUNCTION
{
    ...
    public TEMP IRme() 
    {
        ...
        if (funcArgs != null) {
            argumentList = funcArgs.IRme(argumentList);}

        TEMP dst = TEMP_FACTORY.getInstance().getFreshTEMP();

        if (funcName.equals("PrintInt")) {
            IR.getInstance().Add_IRcommand(new IRcommand_PrintInt(argumentList,IR.getInstance().currLine));   
        } else if (funcName.equals("PrintString")) {
            IR.getInstance().Add_IRcommand(new IRcommand_PrintString(argumentList,IR.getInstance().currLine));
        } else {
            IR.getInstance().Add_IRcommand(new IRcommand_CallFunction(dst,funcName,argumentList,IR.getInstance().currLine));
        }
        return dst;
    }
